...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        '''
            mask:                               anchors which has a matched gt box is 1
            noobj_mask:                         not mask
            t_box:                              gt x,y,w,h on the feature map scale
            t_conf, t_cls:                      gt confidence(1) and class
            box_loss_scale_x, box_loss_scale_y: w,h in range (0,1)
        '''
        # 找到哪些先验框内部包含物体(gt_box中心点落在哪里就由哪个网格负责预测)
        mask, noobj_mask, t_box, tconf, tcls, box_loss_scale_x, box_loss_scale_y = self.get_target(targets,
                                                                                                   scaled_anchors, in_w,
                                                                                                   in_h)
        '''
            noobj_mask         : =0 while (no gt_box's center here and no pred_box here match gt_box successfully)
            pred_boxes_for_ciou: decoded pred_boxes
        '''
        ...
        # focal loss
        # loss_conf = torch.sum(BCEobj(conf, mask) * mask / bs) + torch.sum(BCEobj(conf, mask) * noobj_mask / bs)
        loss_cls = torch.sum(
    ...
    def get_target(self, target, anchors, in_w, in_h):
        ...
        for b in range(bs):
            ...
            for t in range(target[b].shape[0]):
                ...
                # 计算出所有先验框的位置(9) [ 9 * [0, 0, w/s, h/s] ]
                anchor_shapes = torch.FloatTensor(np.concatenate((np.zeros((self.num_anchors, 2)),
                                                                  np.array(anchors)), 1))
                # 计算重合程度
                anch_ious = bbox_iou(gt_box, anchor_shapes)

                # Find the best matching anchor box for each gt box (1v9)
                best_n = np.argmax(anch_ious)
                if best_n not in anchor_index:  # 不在本层负责的3个anchors范围内
                    continue
    ...
    def get_ignore(self, prediction, target, scaled_anchors, in_w, in_h, noobj_mask):
        ...
        # 生成网格，先验框中心，网格左上角
        grid_x = torch.linspace(0, in_w - 1, in_w).repeat(in_w, 1).repeat(int(bs * self.num_anchors / 3), 1, 1).view(
            x.shape).type(FloatTensor)
        grid_y = torch.linspace(0, in_h - 1, in_h).repeat(in_h, 1).t().repeat(int(bs * self.num_anchors / 3), 1,
