...
def fit_ont_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, genval, cuda, log_writer):
    total_loss = 0
    val_loss = 0
    ...
    # save in tensorboard
    log_writer.add_scalar('Train_loss', float(total_loss / (epoch_size + 1)), epoch)
    log_writer.add_scalar('Validation_loss', float(val_loss / (epoch_size_val + 1)), epoch)
...
if __name__ == "__main__":
    ...
    model_path = "logs/weights/FPN/Epoch_111.pth"
    ...
    if True:
        ...
        if Use_Data_Loader:
            train_dataset = YoloDataset(train_lines, (input_shape[0], input_shape[1]), mosaic=False)
            ...
            gen = Generator(Batch_size, train_lines, (input_shape[0], input_shape[1])).generate(mosaic=False)
        ...
        for epoch in range(Init_Epoch, Freeze_Epoch):
            ...
            fit_ont_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, gen_val, Cuda, log_writer)
            lr_scheduler.step()
